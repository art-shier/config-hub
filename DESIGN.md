...
The application shell contains the brand lockup, session controls, a responsive primary navigation region, and a focusable content landmark. Registers use semantic lists and tables, ruled rows, monospaced identifiers, and explicit empty/loading/error states. The configuration register is a conventional fixed-layout Key / Value / Service / Actions table with stable three-rem rows. It keeps a minimum table width and scrolls horizontally on narrow surfaces instead of changing into cards. Configuration cell text is constrained to one line with an ellipsis; only genuinely overflowing text becomes focusable and exposes the exact full value in an app-owned tooltip after a short delay.

### Forms and overlays

Fields are labelled native controls with compact uppercase metadata labels, strong borders, inline error text, and retained drafts. Product textareas use the shared `resize-none` behavior with an adequate default or component-specific minimum height; the canonical form-field minimum remains `7rem`, and longer configuration values retain their established scrollable writing area. Configuration creation and per-row editing share one `ModalDialog` form for Key, exact Value, Service, and change message. Edit mode also owns deletion confirmation. Dirty entry dialogs guard Cancel, Escape, route navigation, and browser unload; validation or mutation failure keeps the draft in place. Native selects are canonical because operating-system-owned popup geometry is accepted. The `datetime-local` control is likewise platform-owned; its browser/OS locale, popup, geometry, and behavior are accepted. `ModalDialog` owns app dialogs with focus placement, Escape behavior, trapping, and focus restoration; product flows do not use browser dialogs.

### Configuration register and tooltip

Configuration is optimized for scanning rather than card presentation. Key uses roughly one quarter of the table, Value receives the largest share, Service remains visible, and Actions has a stable compact width. The complete value remains in application state and API payloads; ellipsis is presentation only. The tooltip opens after `300ms` on pointer hover or keyboard focus, closes after `100ms`, dismisses on Escape, preserves line breaks, and flips above the trigger when necessary. The layer order is dialog (`600`) above backdrop (`500`) above popover (`300`); table tooltips never cover an active dialog.
