...
describe("ProjectsPage", () => {
  ...
  it("ignores a stale save after the dialog is closed and reopened", async () => {
    mockSession({ role: "admin" });
    mockProjects([shop]);
    server.use(
    ...
    const user = userEvent.setup();
    await user.click(await screen.findByRole("button", { name: "New project" }));
    await user.type(screen.getByLabelText("Project slug"), "stale");
    await user.type(screen.getByLabelText("Project name"), "Stale project");
    await user.click(screen.getByRole("button", { name: "Create project" }));
    ...
    await waitFor(() =>
      expect(screen.queryByRole("link", { name: "Stale project" })).not.toBeInTheDocument(),
    );
  });
});
